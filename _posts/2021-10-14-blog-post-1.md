...
```python
cursor = conn.cursor()
cursor.execute("SELECT name FROM sqlite_master WHERE type='table'") #select all table kind variable from out database
...
def query_climate_database(country, year_begin, year_end, month):
    ...
    cmd = \
    """
    SELECT S.name, S.latitude, S.longitude, C.Name, T.Year, T.Month, T.Temp #variable we want to see in the final table
...
def graph_weather_development(year, country):
    df = country_weather_development(year, country)
    fig = px.line(df, x='Month', y="Temp", color = "NAME")
    return fig
```
...
def station_weather_development(year_1, year_2, station):
    ...
    FROM temperatures T
    LEFT JOIN stations S ON T.id = S.id
    WHERE T.Year == ? AND S.NAME = ? OR T.Year == ? AND S.NAME == ? 
    """

    df = pd.read_sql_query(cmd, conn, params = [year_1, station, year_2, station])
    return df
